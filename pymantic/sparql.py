...
# abstract methods, see Select for the idea
    ...
    def execute(self):
        ...
        response, content = http.request(
            uri=uri, method=method, headers=self.headers, body=body)
...
class _Select(_SelectOrUpdate):
    ...
    def execute(self):
        ...
        if format:
            ...
            graph.parse(StringIO(content),
                        self.server.query_url, format=format)
            return graph
...
class UpdateableGraphStore(SPARQLServer):
    ...
    def request_url(self, graph_uri):
        if self.param_style:
            ...
            return urljoin(self.dataset_url, quote_plus(graph_uri))
...
def changeset(a, b, graph_uri):
    ...
    graph.add((change_set, cs["createdDate"], rdflib.Literal(
