...
class NTriplesParser(BaseNParser):
    ...
    def __init__(self, environment=None):
        ...
        self.line = Star(Space()) & Optional(
            ~self.triple | ~self.comment) & ~(Literal('\n'))
...
class ClassicTurtleParser(BaseLeplParser):
    ...
    def __init__(self, environment=None):
        ...
        ucharacter = Or(character_escape,
                        Regexp(r'[\u0020-\u003D\u003F-\u005B\]-\U0010FFFF]')) | r'\>'
        scharacter = Or(character_escape,
                        Regexp(r'[\u0020-\u0021\u0023-\u005B\]-\U0010FFFF]')) | r'\"'
        lcharacter = echaracter | '\"' | '\u0009' | '\u000A' | '\u000D'
        ...
        prefixStartChar = Regexp(r'[A-Z]') | Regexp(r'[a-z]') | Regexp(r'[\u00C0-\u00D6]') | Regexp(r'[\u00D8-\u00F6]') | Regexp(r'[\u00F8-\u02FF]') | Regexp(r'[\u0370-\u037D]') | Regexp(r'[\u037F-\u1FFF]') | Regexp(
            r'[\u200C-\u200D]') | Regexp(r'[\u2070-\u218F]') | Regexp(r'[\u2C00-\u2FEF]') | Regexp(r'[\u3001-\uD7FF]') | Regexp(r'[\uF900-\uFDCF]') | Regexp(r'[\uFDF0-\uFFFD]') | Regexp(r'[\U00010000-\U000EFFFF]')
        nameStartChar = prefixStartChar | "_"
        nameChar = nameStartChar | '-' | Regexp(r'[0-9]') | '\u00B7' | Regexp(
            r'[\u0300-\u036F]') | Regexp(r'[\u203F-\u2040]')
        name = (nameStartChar & nameChar[:])[...] > 'name'
        prefixName = (prefixStartChar & nameChar[:])[...] > 'prefixName'
        language = Regexp(r'[a-z]+ (?:-[a-z0-9]+)*') > 'language'
        qname = ((Optional(prefixName) & ~Literal(':') & Optional(
            name) > dict) > self.resolve_prefix) > 'qname'
        ...
        with Separator(~Star(Any(' \t\n\r'))):
            uriref = (And(~Literal('<'), relativeURI, ~Literal('>'))
                      > self.resolve_relative_uri) > 'uriref'
            ...
            decimal = Regexp(
                r'(?:-|\+)?(?:[0-9]+\.[0-9]*|\.(?:[0-9])+)') > self.make_decimal_literal
            exponent = r'[eE](?:-|\+)?[0-9]+'
            double = Regexp(r'(?:-|\+)?(?:[0-9]+\.[0-9]*' + exponent + r'|\.[0-9]+' +
                            exponent + r'|[0-9]+' + exponent + ')') > self.make_double_literal
            boolean = Literal('true') | Literal(
                'false') > self.make_boolean_literal
            datatypeString = quotedString & "^^" & (
                resource > 'dataType') > self.make_datatype_literal
            literal = Or(datatypeString,
                         quotedString & Optional(
                             ~Literal('@') & language) > self.make_language_literal,
                         double,
                         decimal,
                         integer,
                         boolean)
            ...
            collection = ~Literal(
                '(') & object_[:] & ~Literal(')') > self.make_collection
            objectList = (object_ & (~Literal(',') & object_)
                          [:] > List) > 'objectList'
            predicateObjectList = ((verb & objectList > Node) & (~Literal(';') & (
                verb & objectList > Node))[:] & ~Optional(';') > List) > 'predicateObjectList'
            blank += Or(nodeID, collection,
                        (~Literal('[') & ~Literal(']') > self.make_triples),
                        (~Literal('[') & predicateObjectList &
                         ~Literal(']') > self.make_triples)
                        )
            triples = subject & predicateObjectList > self.make_triples
            base = (~Literal('@base') & Plus(ws) &
                    uriref > dict) > self.record_base
            prefixId = (~Literal('@prefix') & Plus(ws) & Optional(prefixName)
                        & ~Literal(':') & uriref > dict) > self.record_prefix
    ...
    def record_prefix(self, values):
        prefix = values[0]
        self._call_state.prefixes[prefix.get(
            'prefixName', '')] = prefix['uriref']
        return ''
        ...
        for predicate_object_node in triples.get('predicateObjectList', ()):
            ...
            for object_ in predicate_object_node.objectList[0]:
                self._call_state.graph.add(
                    self.env.createTriple(subject, predicate, object_))
        return subject
...
class TurtleParser(BaseLeplParser):
    ...
    def __init__(self, environment=None):
        ...
        PN_CHARS_BASE = Regexp(r'[A-Za-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u02FF'
                               r'\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F'
                               r'\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD'
                               r'\U00010000-\U000EFFFF]') | UCHAR
        ...
        PN_LOCAL = (PN_CHARS_U | Regexp(
            '[0-9]')) & Optional(Star(PN_CHARS | Literal(".")) & PN_CHARS) > ''.join
        ...
        IRI_REF = (~Literal('<') & (Star(Regexp(
            r'[^<>"{}|^`\\\u0000-\u0020]') | UCHAR | ECHAR) > ''.join) & ~Literal('>'))
        ...
        with Separator(intertoken):
            ...
            prefixID = (~Literal('@prefix') & PNAME_NS &
                        IRI_REF) > self.bind_prefixed_name
            ...
            BooleanLiteral = (Literal('true') | Literal(
                'false')) >> self.boolean_value
            ...
            blankNodePropertyList = ~Literal('[') & predicateObjectList & ~Literal(
                ']') > self.make_blank_node_property_list

            collection = (
                ~Literal('(') & object[:] & ~Literal(')')) > self.make_collection
            ...
            objectList = ((object & (~Literal(',') & object)
            ...
            if self.clark('rdf', 'resource') in property_element.attrib:
                object_ = self._resolve_uri(
                    property_element, property_element.attrib[self.clark(
                        'rdf', 'resource')])
                sink.add(Triple(subject, NamedNode(
                    predicate), NamedNode(object_)))
        return subject
